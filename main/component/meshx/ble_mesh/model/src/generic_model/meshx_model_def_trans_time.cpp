/**
 * @file meshx_model_def_trans_time.cpp
 * @brief Implementation of Generic Default Transition Time Model classes for MeshX.
 *        This file contains the implementation of the Generic Default Transition Time Server and Client models
...
/**
 * @brief Handle Generic Default Transition Time state change notifications from the MeshX stack.
 *
 * This function is responsible for handling Generic Default Transition Time state change notifications
 * from the MeshX stack. It is called when the MeshX stack receives a state
 * change event from the Generic Default Transition Time server model. The function publishes the
 * state change event to the control task framework, which in turn notifies the
 * application about the state change.
 *
 * @param[in] param  Pointer to the Generic Default Transition Time client callback parameter structure.
 ...
 *
 * This function is used to create a meshXGenericDefTransTimeClientModel instance based on a BLE device.
 ...
 *
 * @return Pointer to the created meshXGenericDefTransTimeClientModel instance
 */
MESHX_GEN_DEF_TRANS_TIME_CLIENT_MODEL_TEMPLATE_PROTO
meshx_err_t meshXGenericDefTransTimeClientModel MESHX_GEN_DEF_TRANS_TIME_CLIENT_MODEL_TEMPLATE_PARAMS
    :: model_from_ble_cb(
...
)
{
    ...
    if(model_id != MESHX_MODEL_ID_GEN_DEF_TRANS_TIME_CLI)
    {
 ...
 *
 * This function takes a pointer to a meshx_gen_def_trans_time_send_params_t structure as input
 * and sends the corresponding packet to the MeshX stack for processing.
 *
 * @param[in] params Pointer to the meshx_gen_def_trans_time_send_params_t structure containing the parameters
 * @return MESHX_SUCCESS on success, or an error code on failure
 */
MESHX_GEN_DEF_TRANS_TIME_CLIENT_MODEL_TEMPLATE_PROTO
meshx_err_t meshXGenericDefTransTimeClientModel MESHX_GEN_DEF_TRANS_TIME_CLIENT_MODEL_TEMPLATE_PARAMS
    :: model_send(meshx_gen_def_trans_time_send_params_t *params)
{
    meshx_err_t err;
    ...
    if (params->ctx->opcode == MESHX_MODEL_OP_GEN_DEF_TRANS_TIME_GET)
    {
        err = this->get_base_model()->plat_send_msg(&send_params);
    }

    else if (params->ctx->opcode == MESHX_MODEL_OP_GEN_DEF_TRANS_TIME_SET ||
             params->ctx->opcode == MESHX_MODEL_OP_GEN_DEF_TRANS_TIME_SET_UNACK)
    {
        set.def_trans_time_set.trans_time = params->trans_time;
        // Note: Default Transition Time SET operations don't use TID
    ...
    else
    {
        ...
        MESHX_LOGE(MODULE_ID_MODEL_CLIENT, "Invalid opcode for Generic Default Transition Time Client: %04x", params->ctx->opcode);
    }
    return err;
}
/**
...
if (!params|| !params->model || !params->ctx)
    ...
    params->ctx->opcode = MESHX_MODEL_OP_GEN_DEF_TRANS_TIME_STATUS;
    ...
    meshx_gen_server_send_params_t send_params = {
        ...
        .data_len = sizeof(meshx_state_change_gen_def_trans_time_set_t)
    };
    return this->get_base_model()->plat_send_msg(&send_params);
}

/**
 * @brief Callback function for handling BLE mesh events for Generic Default Transition Time Server Model
 *
 * @param[in] p_dev     Pointer to the device structure
    ...
    )
{
    ...
    if(model_id != MESHX_MODEL_ID_GEN_DEF_TRANS_TIME_SRV)
    {
        /* Callback triggered not for this model */
        return MESHX_SUCCESS;
    }
 ...
 *
 * This function handles the platform-specific model creation process for Generic Default Transition Time Server models.
    ...
    :: plat_model_create(MESHX_MODEL* p_plat_model_ptr)
{
    ...
    err = meshx_plat_def_trans_time_gen_srv_create(this->get_plat_model(), &p_pub, &p_gen);
    if(err)
    {
        MESHX_LOGE(MODULE_ID_MODEL_SERVER, "Failed to create Generic Default Transition Time Server Model");
    }
    else
    {
...
/**
 * @brief Deletes the Generic Default Transition Time Server model and its associated resources.
 *
 * This function frees the memory allocated for the Generic Default Transition Time Server
    ...
    :: plat_model_delete(void)
{
    ...
    if (err)
    {
        MESHX_LOGE(MODULE_ID_MODEL_SERVER, "Failed to delete Generic Default Transition Time Server Model");
    }
    else
    {
